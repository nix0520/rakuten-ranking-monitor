...
(async()=>{
  ...
  try{
    ...
    await page.fill('#savedViewName','测试筛选');await page.click('#saveView');
    assert.equal(await page.locator('#savedViewSelect option').count(),2);
    const operationDownload=page.waitForEvent('download');await page.click('#exportOperations');await operationDownload;
    assert.match(await page.locator('#exportOperationsStatus').innerText(),/已导出/);
